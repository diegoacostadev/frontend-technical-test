...
export default function ConversationCard({conversation, user}: {conversation: Conversation, user: number}) {

  const { setActiveConversation } = useChatStore();

  const cDate = new Date(Date.now() - conversation.lastMessageTimestamp);
  const avatar = user == conversation.senderId ? conversation.recipientNickname : conversation.senderNickname;
  ...
  return (
    ...
    <article className="flex items-center gap-x-4 border-b px-4 py-3 text-left">
      <div className="grid h-9 w-9 basis-9 shrink-0 place-content-center rounded-full bg-orange-400">
